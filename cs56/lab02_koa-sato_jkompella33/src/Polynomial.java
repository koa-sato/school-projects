...
/** <p>Polynomial represents a polynomial from algebra with integer 
    ...
    @author P. Conrad
    @author TODO: Koa Sato, Joseph Kompella
...
public class Polynomial extends ArrayList<Integer> {
	...
	for (int i = this.getDegree(); i >= 0; i--) {
	    if (this.get(i) != 0) {
		if (this.get(i) > 0 && i != this.getDegree()) 
		    result += " + ";
		else if (this.get(i) < 0 && i != this.getDegree())
		    result += " - ";
		if (i != this.getDegree())
	            result = addCoefToString(Math.abs(this.get(i)), result, i);
		else if (this.get(i) == -1)
		    result += "-";
		else
		    result = addCoefToString(this.get(i), result, i);
		if (i == 1)
		    result += "x";
		else if (i != 0)
	            result += "x^" + i;
    ...
    public String addCoefToString(int coef, String s, int degree) {
	if (degree == 0)
	    s += "" + coef;
    ...
    */
	...
	if (coeffsHighToLow.length == 1)
	    return coeffsHighToLow;
	int length = degreeCoeffsHighToLow(coeffsHighToLow) + 1;
	int [] stub = new int [length];
	
	for (int i = 0; i < length; i++) {
	    stub[length - 1 - i] = coeffsHighToLow[coeffsHighToLow.length-length+i];
	}
    ...
    */
	...
	if (coeffsLowToHigh.length == 1)
	    return coeffsLowToHigh;  
  	int length = degreeCoeffsLowToHigh(coeffsLowToHigh) + 1;
        int [] stub = new int [length];

        for (int i = 0; i < length; i++) {
            stub[length - i -1] = coeffsLowToHigh[i];
        }
    ...
    */
	...
	Polynomial stub = new Polynomial (new int [p.size()]);
	int shorter  = 0;
	boolean thisIsLonger;
	if (this.size() >= p.size()) {
	    shorter = p.size();
     	    stub = this;
	    thisIsLonger = true;
	...
	if (thisIsLonger) {
	    for (int i = 0; i < shorter; i++) {
	        stub.set(i,stub.get(i) + p.get(i));
	    }
	}
	else {
	    for (int i = 0; i < shorter; i++) {
                stub.set(i, stub.get(i) + this.get(i));
            }
	}
	int[] stubArray = new int[stub.size()];
	for (int i = 0; i < stub.size(); i++)
	    stubArray[i] = stub.get(i);
	stubArray = highToLow(stubArray);
	stubArray = lowToHigh(stubArray);
	stub.clear();
	for (int i = 0; i < stubArray.length; i++)
	    stub.add(stubArray[i]);
	return stub;
    ...
    */
	...
	Polynomial stub = new Polynomial (new int [this.size() + p.size() - 2]);
	for (int i = 0; i < this.size() + p.size() - 2; i++)
	    stub.add(0);
        for (int i = p.getDegree(); i >= 0; i--) {
            for (int j = this.getDegree(); j >= 0; j--) {
                stub.set(i + j, p.get(i) * this.get(j) + stub.get(i + j));
            }
        }
